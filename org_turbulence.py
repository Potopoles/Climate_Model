...
class turbulence:
    ...
    def __init__(self, GR, i_turbulence):
        ...
        self.RHO   = np.full( ( GR.nx, GR.ny, GR.nz  ), np.nan)
        self.RHOVB = np.full( ( GR.nx, GR.ny, GR.nzs ), np.nan)
        self.K_h   = np.full( ( GR.nx, GR.ny, GR.nzs ), np.nan) 
        self.K_h[:,:,1:GR.nzs-1] = 0.1

        self.dzs = np.full( ( GR.nx, GR.ny, GR.nzs ), np.nan)
        self.dz = np.full( ( GR.nx, GR.ny, GR.nz ), np.nan)
    ...
    def turbulent_flux_divergence(self, GR, VAR):
        ...
        FLUXVB = np.zeros( ( GR.nx, GR.ny, GR.nzs ) ) 
