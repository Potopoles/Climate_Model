...
from tendency_functions import  hor_adv, vert_adv, num_dif
...
#                ','+NL.wp_str+','+NL.wp_str+','+NL.wp_str+
#                ','+NL.wp_str+')')
def _launch_on_gpu(dPOTTdt, POTT, UFLX, VFLX, COLP, A,
...
def _compute_tendency(POTT, POTT_im1, POTT_ip1, POTT_jm1, POTT_jp1,
...
_launch_on_gpu_name = _launch_on_gpu.__name__
print(_launch_on_gpu_name)
quit()
...
class Tendency_POTT:
    ...
    def __init__(self, on_GPU):
        ...
        if on_GPU:
            self.hor_adv = njit(hor_adv, device=True, inline=True)
            self.num_dif = njit(num_dif, device=True, inline=True)
            self.vert_adv = njit(vert_adv, device=True, inline=True)
            self._compute_tendency = njit(_compute_tendency, device=True,
                                        inline=True)
            self.launch = cuda.jit('void('+NL.wp_str+','+NL.wp_str+','+NL.wp_str+
                ','+NL.wp_str+','+NL.wp_str+','+NL.wp_str+
                ','+NL.wp_str+','+NL.wp_str+','+NL.wp_str+
                ','+NL.wp_str+')')(_launch_on_gpu)

            
    def compute_tendency(self, ):
        self.launch[bpg, tpb](dPOTTdt, POTT, UFLX, VFLX, COLP, A,
                                    POTTVB, WWIND, COLP_NEW, dsigma)
...
def run(comp_mode):
    ...
    if comp_mode == 2:
        print('gpu')
        Tend_POTT = Tendency_POTT(on_GPU=True)
        Tend_POTT.compute_tendency(dPOTTdt, POTT, UFLX, VFLX, COLP, A,
                                    POTTVB, WWIND, COLP_NEW, dsigma)
        quit()
